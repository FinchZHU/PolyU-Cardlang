...
public class SemanticAnalyzer extends DepthFirstAdapter {
    ...
    public String getPreSequence() {
        ...
        for (int i = 0; i < preObjStack.get(0).size(); i++) {
            preSequence.append(preObjStack.get(0).get(i) + '.');
        }
        String res = preSequence.toString();
        preSequence = new StringBuilder();
        return res;
    }
    ...
    public void outASubscriptId(ASubscriptId node) {
        ...
        if (getNodeDataType(node.getExpr()) != TYPE_INTEGER) {
            reportError("invalid subscript: index is not integer: " + getPreSequence() + node.getExpr().toString() + ": " + getNodeDataType(node.getExpr()));
        }

        // see what the array type is and set the node type to be the corresponding variable type
        if (getIDType(removeArrIndex(getPreSequence() + node.getId().toString())) == TYPE_CARD_ARR) {
            setNodeDataType(node, TYPE_CARD);
        }
        else if (getIDType(removeArrIndex(getPreSequence() + node.getId().toString())) == TYPE_PLAYER_ARR) {
