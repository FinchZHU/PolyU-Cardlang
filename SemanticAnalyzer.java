...
public class SemanticAnalyzer extends DepthFirstAdapter {
    ...
    //
    //
    // Please delete this session before submission.
    //
    //
    //
    //
    ...
    //
    //
    // I forgot why I feel a num expression must be of integer or boolean type.
    // If it is indeed not necessary, please delete the report error session in the outABaseNumexpr()
    //
    //
    //
    //
    ...
    public void outABaseNumexpr(ABaseNumexpr node) {
        if (getNodeDataType(node.getVal()) != TYPE_INTEGER && getNodeDataType(node.getVal()) != TYPE_BOOLEAN) {
            reportError("invalid base num expression: " + node.getVal() + ": " + getNodeDataType(node.getVal()));
        }
    ...
    public void outALengthId(ALengthId node) {
        if (getIDType(node.getObj().toString()) != TYPE_CARD_ARR && getIDType(node.getObj().toString()) != TYPE_PLAYER_ARR) {
            reportError("invalid subscript: not an array: " + node.getObj().toString() + ": " + getIDType(node.getObj().toString()));
        }
        setNodeDataType(node, TYPE_INTEGER);
    }
}
